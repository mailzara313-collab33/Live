...
class PaymentProvider extends ChangeNotifier {
      ...
      try {
        ...
        if (!error) {
          ...
          if (phonepe) {
            ...
            context.read<CartProvider>().phonePeClientId =
                payment['phonepe_client_id'];
            context.read<CartProvider>().phonePeClientSecret =
                payment['phonepe_client_secret'];
          }
          if (cod) {
  ...
  Future<Map<String, dynamic>> payWithPayStack({
    ...
    try {
      ...
      print('Currency used: ${response.reference}');
      if (response.status) {
        return {
          'error': false,
