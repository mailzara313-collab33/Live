...
class AuthenticationProvider extends ChangeNotifier {
    ...
    try {
      if (type == GOOGLE_TYPE) {
        ...
        if (userCredential != null) {
          ...
          throw ApiException('somethingMSg'.translate(context: context));
        }
  ...
  Future<UserCredential?> signInWithApple(BuildContext context) async {
    try {
      ...
      if (errorMessage == 'Null check operator used on a null value') {
        ...
        setSnackbar('CANCEL_USER_MSG'.translate(context: context), context);
  ...
  //get System Policies
  Future<Map<String, dynamic>> getLoginData({String type = 'phone'}) async {
    try {
      ...
      if (type == 'email') {
        // Email login
        parameter = <String, dynamic>{EMAIL: email, PASSWORD: passwordPara};

        result = await AuthRepository.loginWithEmail(parameter: parameter);
      } else {
        // Phone login
        ...
        result = await AuthRepository.fetchLoginData(parameter: parameter);
      }
  ...
  //for login
  Future<Map<String, dynamic>> getVerifyUser(
    String mobile, {
    ...
    try {
      ...
      var result = await AuthRepository.fetchverificationData(
        parameter: parameter,
      );
  ...
  Future<Map<String, dynamic>> getVerifyUserByEmail(
    String email, {
    required bool isForgotPassword,
  }) async {
    try {
      // Determine the type based on the context
      String otpType = isForgotPassword ? 'forgot_password' : 'registration';

      var result = await AuthRepository.sendEmailOtp(
        email: email.trim(),
        type: otpType,
      );
  ...
  Future<Map<String, dynamic>> getSingUPData() async {
    try {
      // Check if this is email registration (countrycode is empty)
      bool isEmailRegistration = countrycode?.isEmpty ?? true;

      Map<String, dynamic> parameter;

      if (isEmailRegistration) {
        // Email registration - use actualMobile for mobile number
        parameter = <String, dynamic>{
          NAME: name,
          EMAIL: singUPemail,
          MOBILE: actualMobile!.replaceAll(' ', ''),
        ...
        if (referCode != null && referCode!.isNotEmpty) {
          parameter[REFERCODE] = referCode;
        }

        var result = await AuthRepository.registerUserWithEmail(
          parameter: parameter,
        );
        return result;
      } else {
  ...
  Future<Map<String, dynamic>> getReset(String mobile) async {
    try {
      var parameter = {MOBILENO: mobile, NEWPASS: newPassword};
  ...
  Future<void> generateReferral(BuildContext context) async {
    String refer = getRandomString(8);
    context.read<AuthenticationProvider>().setreferCode(refer);

    try {
      var data = {REFERCODE: refer};
