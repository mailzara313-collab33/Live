...
import 'package:eshop_multivendor/Screen/Search/Widget/tempTagWidget.dart';
...
class _SearchState extends State<Search> with TickerProviderStateMixin {
  ...
  void _transactionscrollListener() {
          ...
          child: Stack(
            children: [
              ...
              context.read<SearchProvider>().productList[index].availability ==
                      '0'
                  ? Text('OUT_OF_STOCK_LBL'.translate(context: context),
    ...
    if (isNetworkAvail) {
      try {
        if (context.read<SearchProvider>().notificationisloadmore) {
          ...
          print('search parameter---$parameter');
  ...
  Widget _tags() {
    return TempTagWidget(scrollController: scrollController);
  }

  Widget _showContentOfProducts() {
    if (_controller.text == '') {
      ...
      return FutureBuilder<List<String>>(
        ...
        builder: (BuildContext context, AsyncSnapshot<List<String>> snapshot) {
          if (snapshot.connectionState == ConnectionState.done &&
            ...
            List<Product> itemList = [];
            if (oldEntriedForFutureBuilderOfHistory.isNotEmpty) {
              for (int i = 0;
  ...
  Future<void> initSpeechState(int from) async {
    ...
    if (hasSpeech) {
      ...
      setSnackbar('MICROPHONE_PERMISSION_REQUIRED'.translate(context: context),
          context);
    }
  }
  ...
  Widget showSpeechDialog(int from) {
    if (from == 1) {
      ...
      itemBuilder: (BuildContext context, int i) {
        ...
        return ListTile(
          ...
          subtitle: textController!.text.toString().trim().isEmpty ||
              ...
              ? null
              : Text(
                  '${'IN'.translate(context: context)} ${suggestion.catName!}',
                  style: TextStyle(
                    color: Theme.of(context).colorScheme.fontColor,
                    fontFamily: 'ubuntu',
                  ),
          ...
          trailing: textController!.text.toString().trim().isEmpty ||
              ...
              : ClipRRect(
                  ...
                  child: suggestion.image == ''
                      ? SvgPicture.asset(
                          DesignConfiguration.setSvgPath(Assets.placeholderSvg),
                          width: 50,
                          height: 50,
                          fit: BoxFit.cover,
                        )
