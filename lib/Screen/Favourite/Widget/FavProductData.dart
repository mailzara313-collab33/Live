...
import '../../../widgets/customAddCartSection.dart';
...
class _FavProductDataState extends State<FavProductData> {
  ...
  Future<void> addToCart(String qty, int from, BuildContext context
      // List<Product> favList,
      ) async {
    isNetworkAvail = await isNetworkAvailable();
    if (isNetworkAvail) {
        ...
        if (singleSellerOrderSystem) {
          if (CurrentSellerID == '' ||
            ...
            if (from == 1) {
              ...
              if (int.parse(qty) >
                ...
                setSnackbar(
                    '${"Max Quantity is".translate(context: context)}-${int.parse(qty) - 1}',
                    context);
              } else {
                context.read<CartProvider>().updateCartItem(
                      widget.favList[widget.index!].id!,
          ...
          if (from == 1) {
            ...
            context
                ...
                .controllerText[widget.index!]
                .text = qty.toString();
            setSnackbar('Cart Update Successfully'.translate(context: context),
                context);
          }
        }
        context
  ...
  Widget build(BuildContext context) {
    if (widget.index! < widget.favList.length && widget.favList.isNotEmpty) {
      ...
      return showContent(price, off);
    } else {
      return const SizedBox();
    }
  }

  Widget showContent(double price, off) {
                ...
                if (widget.favList[widget.index!].noOfRating! != '0')
                  Positioned(
                      ...
                      child: RatingCartForProduct(
                          noOfRating: widget.favList[widget.index!].noOfRating!,
                          totalRating: widget.favList[widget.index!].rating!)),
      ...
      child: ClipRRect(
        ...
        child: Stack(
          children: [
            ...
            Positioned.fill(
              child: widget.favList[widget.index!].availability == '0'
                  ? Container(
                      height: 55,
  ...
  Widget productInfo(double price, double off) {
    return Expanded(
      child: Padding(
        padding:
            const EdgeInsetsDirectional.only(top: 10.0, start: 5.0, end: 35),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            ...
            Row(
              children: [
                ...
                Text(
                  double.parse(widget.favList[widget.index!].prVarientList![0]
                                  .disPrice!) !=
                              0 &&
                          double.parse(widget.favList[widget.index!]
            ...
            CommonAddCartSection(
              controller: context
                  .read<FavoriteProvider>()
                  .controllerText[widget.index!],
              model: widget.favList[widget.index!],
              onAddToCart: () async {
                await addToCart('1', 1, context);
              },
              onRemoveFromCart: () {
                removeFromCart(
                  widget.index!,
              ...
              itemsCounter: widget.favList[widget.index!].itemsCounter!,
              onQuantitySelected: (String value) async {
                await addToCart(value, 2, context);
              },
              checkFavStatus: true,
            ),
          ],
          ...
          onTap: () {
            if (context.read<UserProvider>().userId != '') {
              Future.delayed(Duration.zero).then(
                (value) => context.read<UpdateFavProvider>().removeFav(
                    widget.favList[widget.index!].id!,
                    widget.favList[widget.index!].prVarientList![0].id!,
                    context),
              );
            } else {
              db.addAndRemoveFav(widget.favList[widget.index!].id!, false);
              context.read<FavoriteProvider>().removeFavItem(
                  widget.favList[widget.index!].prVarientList![0].id!);

              setSnackbar(
                  'Removed from favorite'.translate(context: context), context);
            }
          },
        ),
      ),
