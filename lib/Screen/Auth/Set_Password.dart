...
class _LoginPageState extends State<SetPass> with TickerProviderStateMixin {
  ...
  Future<void> checkNetwork() async {
    ...
    if (isNetworkAvail) {
      // Use email-based password reset API if in email mode
      if (widget.isEmailMode == true && widget.otp != null) {
        try {
          var result = await AuthRepository.resetPasswordWithEmail(
          ...
          if (error == false) {
            setSnackbar(
              'PASS_SUCCESS_MSG'.translate(context: context),
              context,
            );
            ...
            setSnackbar(msg ?? 'FAILED_RESET_PASSWORD'.translate(context: context), context);
          }
        } catch (e) {
          await buttonController!.reverse();
        ...
        // Use existing phone-based password reset API
  ...
  Widget setPass() {
    return Padding(
      ...
      child: TextFormField(
        style: TextStyle(
          color: Theme.of(context).colorScheme.fontColor.withValues(alpha: 0.7),
          fontWeight: FontWeight.bold,
  ...
  Widget setPassBtn() {
    return Center(
      child: Padding(
        ...
        child: AppBtn(
          title: 'SET_PASSWORD'.translate(context: context),
