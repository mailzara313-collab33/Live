...
class _HomePageState extends State<HomePage>
  ...
  // Controller for outer NestedScrollView (Android) & CustomScrollView (iOS)
  final ScrollController _scrollBottomBarController = ScrollController();

  // Separate controller for the inner ListView inside NestedScrollView (Android).
  // Using a separate controller avoids the "multiple ScrollViews attached" error
  // while still letting us receive scroll updates to show/hide the bars instantly.
  final ScrollController _innerListScrollController = ScrollController();
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      user.setCountrycode(setting.countryCode);
      //setUserData();
      ...
      // Attach listener for iOS (CustomScrollView) and the outer controller.
      Future.delayed(Duration.zero).then((value) {
        hideAppbarAndBottomBarOnScroll(_scrollBottomBarController, context);

        // Attach listener to inner ListView controller for Android so that
        // we also get scroll updates when only the body ListView scrolls.
        hideAppbarAndBottomBarOnScroll(_innerListScrollController, context);
      });
    });
    super.initState();
  }
        ...
        onPopInvokedWithResult: (didPop, result) {
          if (Dashboard.dashboardScreenKey.currentState?.selBottom == 0) {
            ...
            if (currentBackPressTime == null ||
              ...
              setSnackbar(
                'Press back again to Exit'.translate(context: context),
                context,
              );
              setState(() {});
            }
              ...
              ? Padding(
                  ...
                  child: NestedScrollView(
                    ...
                    body: RefreshIndicator(
                      ...
                      onRefresh: _refresh,
                      child: ListView(
                        controller: _innerListScrollController,
          ...
          if (!systemConfigData['error']) {
            ...
            if (context.read<UserProvider>().userId != '') {
              context.read<UserProvider>().setCartCount(
                systemConfigData['cartCount'],
              );
              context.read<UserProvider>().setBalance(
                systemConfigData['userBalance'],
              );
              context.read<UserProvider>().setPincode(
                systemConfigData['pinCode'],
              );
              ...
              if (systemConfigData['referCode'] == null ||
                ...
                context.read<UserProvider>().setReferCode(
                  systemConfigData['referCode'],
                );
  ...
  Future<void> generateReferral() async {
    ...
    var parameter = {REFERCODE: refer};

    apiBaseHelper
        .postAPICall(validateReferalApi, parameter)
        .then(
          (getdata) {
            bool error = getdata['error'];
            if (!error) {
  ...
  @override
  void dispose() {
    // Dispose both scroll controllers and remove listeners
    _innerListScrollController.dispose();
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: Theme.of(
                context,
              ).colorScheme.fontColor.withValues(alpha: 0.2),
            ),
          ),
              ...
              Expanded(
                child: InkWell(
                  child: Row(
                    children: [
                      SvgPicture.asset(
                        DesignConfiguration.setSvgPath(Assets.homepageSearch),
                        height: 15,
                        width: 15,
                      ),
                      Padding(
              ...
              Padding(
                ...
                child: InkWell(
                  ...
                  child: SvgPicture.asset(
                    DesignConfiguration.setSvgPath(Assets.voiceSearch),
                    height: 25,
                    width: 25,
